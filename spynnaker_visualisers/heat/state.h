...
static float** history_data;		// this stores the historic value the plotted points (double the initial width should be sufficient)
static float** history_data_set2;	// 2nd set of data for ancillary raster plot window
static float* immediate_data;		// this creates a buffer tally for the Ethernet packets (1 ID = one plotted point)

// ------------------------------------------------------------------------

// GLOBAL VARIABLES, my bad.
static char plotvaluesinblocks = 0;	// set non-zero if you just want the coloured blocks with values, (tiles/histogram only)
static char somethingtoplot = 0;	// determines when we should update the screen (no point in plotting no change eh?)
...
static char needtorebuildmenu = 0;	// if a menu is open we can't reconfigure it. So we queue the request.
static char menuopen = 0;		// a callback populates this as 1 or 0 depened on whether a menu is open or not.
static char editmode = 1, livebox = -1;	// for user feedback - box selection and whether edit mode is toggled on/off.
